...
def barplot(
    ...
    **kwargs,
    ...
    if estimator == "LogMean":
        bp = sns.barplot(
            df, x=x_axis, y="expr", estimator=log_estimator, capsize=capsize, ax=ax, palette=palette, **kwargs
        )
    else:
        bp = sns.barplot(df, x=x_axis, y="expr", estimator=estimator, capsize=capsize, ax=ax, palette=palette, **kwargs)
    ...
    if ctrl_cond is not None and groups_cond is not None:
        if groups_pvals is None:
            testing = TestData(adata, feature, x_axis, ctrl_cond, groups_cond, test, corr_method)
            testing.run_test()
